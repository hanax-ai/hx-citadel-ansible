...
Note: ingest_doc not included (file upload requires multipart/form-data)
...
class MCPServerUser(HttpUser):
    ...
    def lightrag_query(self):
        ...
        with self.client.post(
            "/tools/lightrag_query", json=payload, catch_response=True
        ) as response:
    ...
    def qdrant_find(self):
        ...
        payload = {"query": "AI applications in healthcare", "limit": 5}
        with self.client.post(
            "/tools/qdrant_find", json=payload, catch_response=True
        ) as response:
            if response.status_code == 200:
                response.success()
            else:
    ...
    def qdrant_store(self):
        ...
        payload = {
            ...
            "metadata": {"source": "load_test", "type": "test_data"},
        }
        with self.client.post(
            "/tools/qdrant_store", json=payload, catch_response=True
        ) as response:
            if response.status_code in [200, 202]:
                response.success()
            else:
    ...
    def crawl_web(self):
        ...
        with self.client.post(
            "/tools/crawl_web", json=payload, catch_response=True
        ) as response:
            if response.status_code in [200, 202]:
                response.success()
            else:
    ...
    def get_job_status(self):
        ...
        with self.client.get(
            f"/tools/get_job_status?job_id={job_id}", catch_response=True
        ) as response:
