    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        ...
        if response.status_code in [200, 202]:
            ...
            if "embedding_dimension" in data:
                assert data["embedding_dimension"] == 768, "Expected 768-dim embeddings (nomic-embed-text)"
            
            print(f"✅ TEST-009-1: Vector stored successfully, point_id={data.get('point_id', 'N/A')}")
        else:
            print(f"⚠️  TEST-009-1: Qdrant/Ollama not available (status 500)")
...
async def test_qdrant_find_vectors(mcp_server_url, test_timeout):
    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        ...
        if len(results) > 0:
            ...
            print(f"✅ TEST-009-2: Found {len(results)} results, top score={scores[0]:.3f}")
        else:
            print(f"✅ TEST-009-2: Search completed (no results in empty collection)")
...
async def test_qdrant_store_multiple_vectors(mcp_server_url, test_timeout):
    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        ...
        for i, text in enumerate(texts):
            payload = {
                "text": text,
                "metadata": {"batch": "TEST-009-3", "index": i}
            }
        ...
        assert len(point_ids) == len(texts), f"Only {len(point_ids)}/{len(texts)} stored successfully"
...
async def test_qdrant_find_no_results(mcp_server_url, test_timeout):
    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        ...
        print(f"✅ TEST-009-4: No results query handled correctly ({len(results)} results)")
...
async def test_qdrant_store_empty_text(mcp_server_url, test_timeout):
    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        ...
        print(f"✅ TEST-009-5: Empty text properly rejected")
...
async def test_qdrant_store_ollama_unavailable(mcp_server_url, test_timeout):
    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        ...
        assert "ollama" in error_text or "embedding" in error_text or "unavailable" in error_text
        
        print(f"✅ TEST-009-6: Ollama unavailable error handled")
...
async def test_qdrant_store_qdrant_unavailable(mcp_server_url, test_timeout):
    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        ...
        assert "error" in data or "message" in data
        error_text = str(data).lower()
        assert "qdrant" in error_text or "vector" in error_text or "connection" in error_text
        
        print(f"✅ TEST-009-7: Qdrant unavailable error handled")
