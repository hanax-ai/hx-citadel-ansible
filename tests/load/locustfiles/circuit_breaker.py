    ...
    def test_tool_with_circuit_breaker(self):
        ...
        with self.client.post(
            ...
            if response.status_code == 200:
                ...
                # Circuit breaker open - check for fast fail
                # Look for circuit breaker indicators in headers or JSON
                is_circuit_breaker = False
                
                # Check headers first
                if 'X-Circuit-Breaker' in response.headers:
                ...
                # Check JSON response
                    ...
                    try:
                        data = response.json()
                        if data.get('error') == 'circuit_open' or data.get('circuitBreaker'):
                            is_circuit_breaker = True
                    except Exception:
                        pass
                # Fallback to text search (more specific)
                elif 'circuit breaker' in response.text.lower() or 'circuit_open' in response.text.lower():
                    is_circuit_breaker = True
                
                if is_circuit_breaker and response_time < 100:
                    response.success()
                    events.request.fire(
                        request_type="POST",
