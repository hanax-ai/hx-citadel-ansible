...
async def test_lightrag_query_hybrid_mode(mcp_server_url, test_timeout):
    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        payload = {"query": "What is machine learning?", "mode": "hybrid"}
        ...
        if response.status_code == 200:
            ...
            print("✅ TEST-011-1: Hybrid mode query successful")
        elif response.status_code == 202:
            data = response.json()
            assert "job_id" in data
            print(
                f"✅ TEST-011-1: Hybrid mode query accepted (async), job_id={data['job_id']}"
            )
        else:
            print("⚠️  TEST-011-1: LightRAG not initialized (status 500)")
...
async def test_lightrag_query_local_mode(mcp_server_url, test_timeout):
    ...
    """
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        payload = {"query": "AI applications in healthcare", "mode": "local"}
        ...
        if response.status_code in [200, 202]:
            print("✅ TEST-011-2: Local mode query successful")
        else:
            print("⚠️  TEST-011-2: LightRAG not available")
...
async def test_lightrag_query_global_mode(mcp_server_url, test_timeout):
    ...
    """
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        payload = {"query": "Explain neural networks", "mode": "global"}
        ...
        if response.status_code in [200, 202]:
            print("✅ TEST-011-3: Global mode query successful")
        else:
            print("⚠️  TEST-011-3: LightRAG not available")
...
async def test_lightrag_query_naive_mode(mcp_server_url, test_timeout):
    ...
    """
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        payload = {"query": "Deep learning basics", "mode": "naive"}
        ...
        if response.status_code in [200, 202]:
            print("✅ TEST-011-4: Naive mode query successful")
        else:
            print("⚠️  TEST-011-4: LightRAG not available")
...
async def test_lightrag_query_context_only(mcp_server_url, test_timeout):
    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        ...
        if response.status_code == 200:
            ...
            print("✅ TEST-011-5: Context-only query successful")
        else:
            print("⚠️  TEST-011-5: LightRAG not available")
...
async def test_lightrag_query_invalid_mode(mcp_server_url, test_timeout):
    ...
    """
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        payload = {"query": "Test query", "mode": "invalid_mode_xyz"}
        ...
        print("✅ TEST-011-6: Invalid mode properly rejected")
...
async def test_lightrag_query_empty_query(mcp_server_url, test_timeout):
    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        ...
        print("✅ TEST-011-7: Empty query properly rejected")
