...
async def test_crawl_web_basic_happy_path(mcp_server_url, test_timeout):
    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        ...
        if response.status_code == 202:
            ...
            print(f"✅ TEST-004-1: Basic crawl completed synchronously")
...
async def test_crawl_web_single_page(mcp_server_url, test_timeout):
    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        ...
        print(f"✅ TEST-004-2: Single page crawl successful")
...
async def test_crawl_web_invalid_url(mcp_server_url, test_timeout):
    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        ...
        assert "error" in data or "detail" in data, "Error response missing error details"
        print(f"✅ TEST-004-3: Invalid URL properly rejected")
...
async def test_crawl_web_unreachable_website(mcp_server_url, test_timeout):
    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        ...
        data = response.json()
        assert "error" in data or "message" in data, "Should return error for unreachable site"
        print(f"✅ TEST-004-4: Unreachable website handled gracefully")
...
async def test_crawl_web_circuit_breaker_protection(mcp_server_url, test_timeout):
    ...
    async with httpx.AsyncClient(timeout=test_timeout) as client:
        ...
        print(f"✅ TEST-004-5: Circuit breaker protection verified (fast-fail in {elapsed:.3f}s)")
